...
# Set up the fonts.
#lo sfondo viene colorato interamente di bianco
...
#varibile globale che definisce il font di default per tutto il gioco
...
pygame.display.update()
#aspetta che il giocatore prema un tasto per iniziare il gioco o per uscire
...
#                    new_cattivi = []
    ...
    if removeCattivo == REMOVECATTIVO:
        removeCattivo = 0
        windowSurface.blit(backgroundStretchedImage,windowSurfaceRectangle)
        pygame.display.update()
    
    if buonoCounter == NEWBUONO:
        buonoCounter = 0
