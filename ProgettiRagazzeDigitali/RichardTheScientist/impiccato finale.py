...
correctLetters = []
missedLetters = []
words = ['amminoacil', 'sintetasi', 'drosophila', 'melanogaster', 'neurospora', 'crassa', 'metionina', 'dissociazione',  'reticolo', 'endoplasmatico', 'fenilchetonuria', 'praseodimio']
...
while True:
    ...
    if parolaIndovinata.find('_') == -1:
        ...
        drawText(correctAnswer[nCorrectAnswer] + ' la parola corretta è ' + secretWord, font, windowSurface, 650, WINDOW_HEIGHT/3 + 150)
        score=score+300
    ...
    if len(missedLetters) == len(HANGMAN_PICS):
        ...
        drawText('Hai finito i tentativi!\n Dopo ' + str(len(missedLetters)) + ' lettere sbagliate e ' + str(len(correctLetters)) + ' corrette, la parola era "' + secretWord + '"', font,windowSurface, 650, WINDOW_HEIGHT/3 + 150)
        drawText('Punteggio: '+str(score), font, windowSurface, 650, WINDOW_HEIGHT/3 + 250)
