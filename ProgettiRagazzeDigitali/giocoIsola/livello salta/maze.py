# -*- coding: utf-8 -*-
"""
Created on Wed Jun 26 09:02:41 2019
...
playerImage = pygame.image.load('esploratore2.png')
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == KEYUP:
            ...
            if event.key == K_DOWN:
                moveDown = False
    if moveDown and player.bottom < 800 and ((windowSurface.get_at(player.bottomleft) != BLACK) and (windowSurface.get_at(player.bottomright) != BLACK)):
        player.top = player.top + MOVE_SPEED
        print(windowSurface.get_at(player.bottomleft), windowSurface.get_at(player.bottomright))
    if moveUp and player.top > 0 and ((windowSurface.get_at(player.topleft) != BLACK) and (windowSurface.get_at(player.topright) != BLACK)):
        player.top = player.top - MOVE_SPEED
    ...
    if moveLeft and player.left > 0 and ((windowSurface.get_at(player.bottomleft) != BLACK) and (windowSurface.get_at(player.topleft) != BLACK)):
        ...
        print(player.x, player.y)
    if moveRight and player.right < 800 and ((windowSurface.get_at(player.bottomright) != BLACK) and (windowSurface.get_at(player.topright) != BLACK)):
