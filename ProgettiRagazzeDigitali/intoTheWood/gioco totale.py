...
fontTitolo = pygame.font.SysFont('forte', 55)
fontNomi = pygame.font.SysFont(None, 30)
...
drawText('INTO THE WOOD', fontTitolo, windowSurface, 180, 376)
...
ostacoloQImage = pygame.image.load('ninfeedef.png')
ostacoloQStretchedImage = pygame.transform.scale(ostacoloQImage, (NINFEALENGTH, NINFEALENGTH))
ostacoloQRect = ostacoloQStretchedImage.get_rect()
ostacoloRImage = pygame.image.load('ninfeedef.png')
ostacoloRStretchedImage = pygame.transform.scale(ostacoloRImage, (NINFEALENGTH, NINFEALENGTH))
ostacoloRRect = ostacoloRStretchedImage.get_rect()
...
ostacoloQ = pygame.draw.rect(windowSurface, BROWN, (275, 700, 50, 50))
ostacoloR = pygame.draw.rect(windowSurface, BROWN, (450, 500, 50, 50))

pesciScore = 0

while True:
    ostacoli = [ostacoloA, ostacoloB, ostacoloC, ostacoloD, ostacoloE, ostacoloF, ostacoloG, ostacoloH, ostacoloI, ostacoloJ, ostacoloK, ostacoloL, ostacoloM, ostacoloN, ostacoloP, ostacoloQ, ostacoloR]
    pesceLista = []
    ...
    ostacoloQRect.topleft = (275, 700)
    ostacoloRRect.topleft = (450, 500)
    ...
    while True: 
        ...
        windowSurface.blit(ostacoloQStretchedImage, ostacoloQRect)
        windowSurface.blit(ostacoloRStretchedImage, ostacoloRRect)
    ...
    if colpireOstacolo(playerRect, ostacoli):
            ...
            gameOverSound.play()
            drawText('GAME OVER', font, windowSurface, 305
            , (WINDOWHEIGHT / 3))
...
drawText('(attento a non toccare le pareti)', font, windowSurface, 150, 171)
...
while True:
    ...
    if colpireLabirinto(playerRect,linee):
        pygame.mixer.music.stop()
        gameOverSound.play()
        drawText('GAME OVER', font, windowSurface, 305, (WINDOWHEIGHT/3))
        drawText('Premi un tasto per rigiocare.', font, windowSurface, (WINDOWWIDTH / 3) - 80, (WINDOWHEIGHT / 3) + 50)
        pygame.display.update()
        aspettaPremaTasto()
...
pygame.mixer.music.stop()
drawText('BRAVO!', font, windowSurface, 340, (WINDOWHEIGHT / 3))
drawText('Hai trovato la legna!', font, windowSurface, 245, (WINDOWHEIGHT / 3) + 48)
pygame.display.update()
...
drawText('(arriva a 400 punti per vincere)', font, windowSurface, 160, 123)
drawText('PREMI UN TASTO PER PARTIRE.', font, windowSurface, 150, 680)
...
playerImage = pygame.image.load('boscarocce.png')
playerRect = playerImage.get_rect()
...
backgroundImage = pygame.image.load('baita.png')
backgroundStretchedImage = pygame.transform.scale(backgroundImage,(WINDOWWIDTH, WINDOWHEIGHT))
windowSurfaceRectangle = windowSurface.get_rect()
windowSurface.blit(backgroundStretchedImage, windowSurfaceRectangle)

drawText('Complimenti, gioco completato!', font, windowSurface, 200, 730)
drawText('Creato da Matilde De Carli, Anna Gabbanini, Martina Serra, Margherita Zannoni', fontNomi, windowSurface, 10, 778)

pygame.mixer.music.load('vittoria.mp3')
pygame.mixer.music.play(-1, 0.0)
