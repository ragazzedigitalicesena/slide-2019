...
drawTextMonti('SCALA LA MONTAGNA E EVITA LE ROCCE!', font, windowSurface, 100, 100)  # vedi funzione sopra (stampa il testo)
drawTextMonti('PREMI UN TASTO PER PARTIRE.', font, windowSurface, 155, 700)
drawTextMonti('Per muoverti potrai usare le freccette ', font, windowSurface, 132, 350)
drawTextMonti('o i seguenti tasti: a per andare a sinistra,', font, windowSurface, 112, 380)
drawTextMonti('s per scendere, d per andare a destra, w per salire.', font, windowSurface, 50, 410)
...
while True: # ogni volta che l'utente ricomincia a giocare
    ...
    playerRectMonti.topleft = (WINDOWWIDTH / 2, WINDOWHEIGHT - 66)  #giocatore al centro nella parte inferiore
...
#  codice che dice a python cosa fare quando il giocatore preme i tasti 
        ...
        if moveLeft and playerRectMonti.left > 0:
            playerRectMonti.move_ip(-1 * PLAYERMOVERATE, 0)
        if moveRight and playerRectMonti.right < WINDOWWIDTH:
            playerRectMonti.move_ip(PLAYERMOVERATE, 0)
        if moveUp and playerRectMonti.top > 0:
            playerRectMonti.move_ip(0, -1 * PLAYERMOVERATE)
        if moveDown and playerRectMonti.bottom < WINDOWHEIGHT:
            playerRectMonti.move_ip(0, PLAYERMOVERATE)
    
    
        if colpireRoccia(playerRectMonti, rocce) or score == SCOREWINNER:
            break
        elif not colpireRoccia(playerRectMonti, rocce) and score != SCOREWINNER:
            for b in rocce:
        ...
        windowSurface.blit(playerImageMonti, playerRectMonti)
    ...
    if colpireRoccia(playerRectMonti, rocce):
        pygame.mixer.music.stop()
        gameOverSound.play()
        drawTextMonti('GAME OVER', font, windowSurface, 325
                 , (WINDOWHEIGHT / 3))
        drawTextMonti('Premi un tasto per rigiocare.', font, windowSurface, (WINDOWWIDTH / 3) - 80, (WINDOWHEIGHT / 3) + 50)
...
pygame.mixer.music.stop()
drawTextMonti('BRAVO!', font, windowSurface, 350, (WINDOWHEIGHT / 3))
drawTextMonti('Hai raggiunto la cima della montagna!', font, windowSurface, 140, (WINDOWHEIGHT / 3) + 50)
